...
namespace UserAutomation {
	...
	void Hook(void* addin)
	{
		if (auto h = LoadHookLibrary()) {
			auto proc = (StartHookProc)GetProcAddress(h, "HookUserAutomation");
			if (proc) proc(addin);
		}
	}
	...
	{
		if (auto h = LoadHookLibrary()) {
			auto proc = (StopHookProc)GetProcAddress(h, "UnookUserAutomation");
			if (proc) proc();
		}
	}
}
...
extern "C" {
	using namespace UserAutomation;

	__declspec(dllexport) void __cdecl UnookUserAutomation()
	{
		auto hWnd = FindWindow(wsHookerName, NULL);
	...
	__declspec(dllexport) void __cdecl HookUserAutomation(void* addin)
	{
