...
верхнего уровня: дескриптор окна, наименование класса и заголовок окна.
...
которая содержит информацию о клиенте тестирования, найденному по номеру порта, 
...
Если СтруктураСвойствПроцесса <> Неопределено Тогда
	...
	ЗаголовокОкна = СтруктураСвойствПроцесса.Title;
	ИдентификаторПроцесса = СтруктураСвойствПроцесса.ProcessId;
	ДатаСтартаПроцесса = Дата(СтруктураСвойствПроцесса.CreationDate);
	КоманднаяСтрока = СтруктураСвойствПроцесса.CommandLine;
КонецЕсли;
```
