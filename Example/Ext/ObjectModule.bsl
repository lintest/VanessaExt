...
Процедура Тест_ФункционалGit() Экспорт
	...
	Ожидается.Тест("Статус рабочей области").Что(ВК).Функц("status").Успешно().Результат().Получить("work").Функц("Получить", 0).Получить("new_name").Равно(ИмяФайла);
	Ожидается.Тест("Добавление в индекс").Что(ВК).Функц("add", ИмяФайла).Успешно();
	Ожидается.Тест("Статус после добавления").Что(ВК).Функц("status").Успешно().Результат().Получить("index").Функц("Получить", 0).Получить("new_name").Равно(ИмяФайла);
	Ожидается.Тест("Первый коммит").Что(ВК).Функц("commit", "Инициализация").Успешно();
	Коммит = Ожидается.Тест("Информация о коммите").Что(ВК).Функц("info", "HEAD").Успешно().Результат().Значение();
	...
	Ожидается.Тест("Получение истории").Что(ВК).Функц("history").Успешно().Результат().ИмеетТип("Массив").Функц("Количество").Равно(2);
	...
	Ожидается.Тест("Статус после удаления").Что(ВК).Функц("status").Успешно().Результат().Получить("work").Функц("Получить", 0).Получить("new_name").Равно(ИмяФайла);
	Ожидается.Тест("Отмена изменений").Что(ВК).Функц("discard", ИмяФайла).Успешно();
	Ожидается.Тест("Статус после отмены").Что(ВК).Функц("status").Успешно().Результат().Равно(Неопределено);
	...
	Ожидается.Тест("Указатель HEAD").Что(ВК).Получить("head").Успешно().Результат().Равно("refs/heads/develop");
	Ожидается.Тест("Переключение ветки").Что(ВК).Функц("checkout", "master").Успешно();
	Ожидается.Тест("Указатель HEAD").Что(ВК).Получить("head").Успешно().Результат().Равно("refs/heads/master");
