...
Процедура Тест_ЗапускПриложений(Ожидается) Экспорт
	...
	ИдентификаторОкна = Ожидается.Тест("Идентификатор окна").Что(ВК).Проц("Пауза", 100).Функц("ПолучитьСписокОкон", ИдентификаторПроцесса).Получить(0, "Window").Вернуть();
	Ожидается.Тест().Что(ВК).Проц("АктивироватьОкно", ИдентификаторОкна);
	...
	РазмерыСовпали = Истина
		И (Размеры.Left + 100 = НовыеРазмеры.Left)
		И (Размеры.Top + 100 = НовыеРазмеры.Top)
		И (Размеры.Width = НовыеРазмеры.Width)
		И (Размеры.Height = НовыеРазмеры.Height)
		И (Размеры.Right + 100 = НовыеРазмеры.Right)
		И (Размеры.Bottom + 100 = НовыеРазмеры.Bottom);
		
	Ожидается.Тест("Размеры и позиция окна совпали").Что(РазмерыСовпали).ЕстьИстина();
	
	Ожидается.Тест().Что(ВК).Проц("АктивироватьОкно", ИдентификаторОкна);
	Ожидается.Тест("Эмуляция ввода текста").Что(ВК).Проц("Пауза", 100).Проц("ЭмуляцияВводаТекста", ПримерТекста, 50);
	Ожидается.Тест("Вызов диалогового окна записи файла").Что(ВК).Проц("Пауза", 100).Проц("ЭмуляцияНажатияКлавиши", "[17, 83]");
	Ожидается.Тест("Ввод имени файла в диалоге").Что(ВК).Проц("Пауза", 100).Проц("ЭмуляцияВводаТекста", ПутьФайла, 50);
	Ожидается.Тест("Закрытие диалога записи файла").Что(ВК).Проц("Пауза", 100).Проц("ЭмуляцияНажатияКлавиши", "[13]");
	Ожидается.Тест("Закрытие приложения").Что(ВК).Проц("Пауза", 100).Проц("ЭмуляцияНажатияКлавиши", "[18, 115]").Проц("Пауза", 100);
	Ожидается.Тест().Что(ВК).Функц("НайтиФайлы", ВременнаяПапка, "*.*", ПримерТекста, Ложь).Получить(0, "name").Равно(ИмяФайла);
