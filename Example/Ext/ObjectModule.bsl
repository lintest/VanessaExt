...
Процедура Тест_ФункционалGit() Экспорт
	...
	Результат = Ожидается.Тест().Что(ВК).Получить("remotes").ЗначениеJSON();
	Репозитории = Ожидается.Тест("Удаленные репозитории").Что(Результат).Получить("result").ИмеетТип("Массив").Значение();
	Ожидается.Тест("Адрес репозитория").Что(Репозитории).Функц("Получить", 0).Получить("url").Равно(АдресКлон);
	...
	Статус = Ожидается.Тест("Статус рабочей области").Что(ВК).Функц("status").Успешно().ЗначениеJSON();
	Ожидается.Тест("Имя нового файла").Что(Статус).Получить("result").Получить("work").Функц("Получить", 0).Получить("new_name").Равно(ИмяФайла);
	Ожидается.Тест("Добавление в индекс").Что(ВК).Функц("add", ИмяФайла).Успешно();
	Статус = Ожидается.Тест("Статус после добавления").Что(ВК).Функц("status").Успешно().ЗначениеJSON();
	Ожидается.Тест("Имя добавленного файла").Что(Статус).Получить("result").Получить("index").Функц("Получить", 0).Получить("new_name").Равно(ИмяФайла);
	Ожидается.Тест("Первый коммит").Что(ВК).Функц("commit", "Инициализация").Успешно();
	Коммит = Ожидается.Тест("Информация о коммите").Что(ВК).Функц("info", "HEAD").Успешно().ЗначениеJSON();
	Ожидается.Тест("Автор коммита").Что(Коммит).Получить("result").Получить("authorName").Равно("Автор");
	Ожидается.Тест("Комиттер коммита").Что(Коммит).Получить("result").Получить("committerName").Равно("Комиттер");
	Ожидается.Тест("Закрытие репозитория").Что(ВК).Функц("close").Успешно();
	...
	История = Ожидается.Тест("Получение истории").Что(ВК).Функц("history").Успешно().ЗначениеJSON();
	Ожидается.Тест("Проверка количества коммитов").Что(История).Получить("result").ИмеетТип("Массив").Функц("Количество").Равно(2);
	Ожидается.Тест("Создание новой ветки").Что(ВК).Функц("checkout", "develop", Истина).Успешно();
	Ветки = Ожидается.Тест("Список веток").Что(ВК).Получить("branches").ЗначениеJSON();
	Ожидается.Тест("Проверка количества веток").Что(Ветки).Получить("result").ИмеетТип("Массив").Функц("Количество").Равно(2);
	...
	Статус = Ожидается.Тест("Статус после удаления").Что(ВК).Функц("status").Успешно().ЗначениеJSON();
	Ожидается.Тест("Имя нового файла").Что(Статус).Получить("result").Получить("work").Функц("Получить", 0).Получить("new_name").Равно(ИмяФайла);
	Ожидается.Тест("Отмена изменений").Что(ВК).Функц("discard", ИмяФайла).Успешно();
	Статус = Ожидается.Тест("Статус после отмены").Что(ВК).Функц("status").Успешно().ЗначениеJSON();
	Ожидается.Тест("Статус должен быть пустой").Что(Статус).Получить("result").Равно(Неопределено);
	...
	Статус = Ожидается.Тест("Указатель HEAD").Что(ВК).Получить("head").Успешно().ЗначениеJSON();
	Ожидается.Тест("Проверка указателя").Что(Статус).Получить("result").Равно("refs/heads/develop");
	Ожидается.Тест("Переключение ветки").Что(ВК).Функц("checkout", "master").Успешно();
	Статус = Ожидается.Тест("Указатель HEAD").Что(ВК).Получить("head").Успешно().ЗначениеJSON();
	Ожидается.Тест("Проверка указателя").Что(Статус).Получить("result").Равно("refs/heads/master");
