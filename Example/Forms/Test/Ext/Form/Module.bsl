...
Процедура ВыполнитьТесты()
	...
	ТестВычислить(Группа, "ПолучитьСвойстваПроцесса(PID)", "ВК.ПолучитьСвойстваПроцесса(ВК.ИдентификаторПроцесса)", "JSON(Значение).ProcessId = ВК.ProcessId");
	ТестВычислить(Группа, )"ПолучитьСписокДисплеев()", "ВК.ПолучитьСписокДисплеев()");
	ТестВычислить(Группа, "ПолучитьСписокДисплеев(hWnd)", "ВК.ПолучитьСписокДисплеев(ВК.АктивноеОкно)");
	ТестВычислить(Группа, "ПолучитьСвойстваДисплея(hWnd)", "ВК.ПолучитьСвойстваДисплея(ВК.АктивноеОкно)");
