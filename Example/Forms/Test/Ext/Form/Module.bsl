...
Процедура ВыполнитьТесты()
	...
	Подкаталог = ВременнаяПапка + "test";
	СоздатьКаталог(ВременнаяПапка);
	...
	ТестВыполнить(Группа, "author", "git.SetAuthor(""Автор"", ""author@lintest.ru"")"); 
	ТестВыполнить(Группа, "committer", "git.SetCommitter(""Комиттер"", ""committer@lintest.ru"")"); 
	...
	ТестВычислить(Группа, "commit", "git.commit(""Инициализация"")", "JSON(Значение).success");
	ТестВычислить(Группа, "info", "git.info(""HEAD"")", "JSON(Значение).result.authorName = ""Автор""");
	ТестВычислить(Группа, "close", "git.close()", "JSON(Значение).success");
	
	ИмяФайла = "текст.txt";
	ПолноеИмя = ВременнаяПапка + ИмяФайла;
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.ДобавитьСтроку("Первая строка");
	ТекстовыйДокумент.ДобавитьСтроку("Вторая строка");
	ТекстовыйДокумент.ДобавитьСтроку("Конец файла");
	ТекстовыйДокумент.Записать(ПолноеИмя, КодировкаТекста.UTF8);
	
	ТестВычислить(Группа, "find", "git.find(""" + Подкаталог + """)", "JSON(Значение).success");
	ТестВычислить(Группа, "open", "git.open(JSON(Значение).result)", "JSON(Значение).success");
	ТестВычислить(Группа, "status", "git.status()", "JSON(Значение).result.work[0].new_name = ""текст.txt""");
	ЗаписатьГруппуТестов(Группа);
