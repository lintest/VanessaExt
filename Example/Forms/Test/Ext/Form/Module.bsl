&НаСервере
Перем ТекущаяГруппа, ТекущаяСтрока, ПроверяемоеЗначение, ЕстьПроблема, ЕстьОшибка, ТекущийТестПрерван;
...
Функция Получить(Знач Имя1 = Неопределено, Знач Имя2 = Неопределено, Знач Имя3 = Неопределено, Знач Имя4 = Неопределено, Знач Имя5 = Неопределено) Экспорт
	...
	Исключение
		Возврат ПрерватьТест(ПроверяемоеЗначение, "Не удалось получить значение по индексу: " + Строка(Имя1));
	КонецПопытки;
...
Функция Равно(Знач Значение) Экспорт
	...
	Если ПроверяемоеЗначение <> Значение Тогда
		ЗаписатьПроблему("Проверяемое значение не соответствует эталону: " + Строка(Значение));
	КонецЕсли;
