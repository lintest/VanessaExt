...
&НаКлиенте
Функция Макет(Знач ИмяМакета) Экспорт

	Если ПустаяСтрока(ТекущийШаг.Наименование) Тогда
		ТекущийШаг.Наименование = "Получить макет: " + ИмяМакета;
	КонецЕсли;

	Попытка
		ПроверяемоеЗначение = ОбъектВладелец.ПолучитьМакет(ИмяМакета);
	Исключение
		Возврат ПрерватьТест(ИмяМакета, "Не удалось получить макет """ + ИмяМакета + """");
	КонецПопытки;
