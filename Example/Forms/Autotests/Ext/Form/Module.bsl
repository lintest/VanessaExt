...
Процедура Тест_УправлениеПриложением(Ожидается) Экспорт
	...
	Команда = "charmap.exe";
	ИдентификаторПроцесса = Ожидается.Тест().Что(ВК).Функц("СоздатьПроцесс", Команда).Вернуть();
	ИдентификаторОкна = Ожидается.Тест("Идентификатор oкна").Что(ВК).Проц("Пауза", 1000).Функц("ПолучитьСписокОкон", ИдентификаторПроцесса).Получить(0, "Window").Вернуть();
	ЗаголовокОкна = Ожидается.Тест().Что(ВК).Функц("ПолучитьСвойстваОкна", ИдентификаторОкна).Получить("Title").Вернуть();
	ЭлементыИнтерфейса = Ожидается.Тест().Что(ВК).Функц("ПолучитьЭлементы", ИдентификаторПроцесса).Получить("Tree").Вернуть();
	...
	Ожидается.Тест("Получить значение элемента").Что(ВК).Функц("НайтиЭлементы", ИдентификаторПроцесса, ИмяТекста, "Edit").Получить(0, "Value").Сокр().Равно("!");

//	НовоеЗначение = "НовоеЗначениеЭлемента";
//	Ожидается.Тест("Установить значение элемента").Что(ВК).Функц("УстановитьЗначениеЭлемента", ИдентификаторТекста, НовоеЗначение).ЕстьИстина();
//	Ожидается.Тест("Получить значение элемента").Что(ВК).Функц("НайтиЭлементы", ИдентификаторПроцесса, ИмяТекста, "Edit").Получить(0, "Value").Сокр().Равно(НовоеЗначение);
	...
	Для каждого ЭлементКоллекции Из ЭлементыИнтерфейса Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Ожидается.Тест().Что(ВК).Функц("НайтиЭлементы", ИдентификаторПроцесса, ИмяКнопки, КнопкаЗакрытия.Type, ИдентификаторРодителя).Получить(0, "Name").Равно(ИмяКнопки);
	Ожидается.Тест().Что(ВК).Функц("НайтиЭлементы", ИдентификаторПроцесса, ИмяКнопки, КнопкаЗакрытия.Type).Получить(0, "Name").Равно(ИмяКнопки);
	Ожидается.Тест().Что(ВК).Функц("НайтиЭлементы", ИдентификаторПроцесса, ИмяКнопки, , ИдентификаторРодителя).Получить(0, "Name").Равно(ИмяКнопки);
	Ожидается.Тест().Что(ВК).Функц("НайтиЭлементы", ИдентификаторПроцесса, ИмяКнопки).Получить(0, "Window").Равно(ЗаголовокОкна);
