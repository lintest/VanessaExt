...
//		ЮнитТест.Добавить("Тест_КлиентWebSocket", "Клиент WebSocket");
	...
	ЮнитТест.Добавить("Тест_ТегиПередСекциейПеременные", "Теги перед секцией Переменные");
...
&НаКлиенте
Процедура Тест_ТегиПередСекциейПеременные(Ожидается) Экспорт
	// Регрессионный тест: feature-level теги, написанные перед блоком "Переменные:",
	// должны наследоваться Функционалом, а не уезжать в Variables-элемент.
	// До фикса: tags = std::move(lexer.tagStack) в конструкторе GherkinElement
	// забирал все теги в первый создаваемый элемент верхнего уровня — Variables.
	// Соответственно feature.tags оказывался пустым, и фильтрация по тегам
	// (например @uf-part2 для разделения сборки на 2 части) переставала работать.
	...
	|	| 'один' | '1'        |
	|
	|Функциональность: Функционал с тегами перед блоком Переменные
	|
	|Сценарий: Простой сценарий
	|	Допустим я пользователь
	|	И я делаю что-то
	|";

	ДанныеФайла = Ожидается.Тест("Парсинг строки").Что(ВК).Функц("ПрочитатьТекст", ТекстСценария).JSON().Вернуть();

	// Главная проверка: все 3 тега должны попасть в feature.tags.
	Ожидается.Тест("Количество feature-тегов").Что(ДанныеФайла).Получить("feature", "tags").Функц("Количество").Равно(3);
	Ожидается.Тест("Тег @uf-part2").Что(ДанныеФайла).Получить("feature", "tags", 0, "text").Равно("uf-part2");
	Ожидается.Тест("Тег @IgnoreOn82Builds").Что(ДанныеФайла).Получить("feature", "tags", 1, "text").Равно("IgnoreOn82Builds");
	Ожидается.Тест("Тег @IgnoreOnOFBuilds").Что(ДанныеФайла).Получить("feature", "tags", 2, "text").Равно("IgnoreOnOFBuilds");

	// Дополнительная проверка: variables не должен иметь этих тегов
	// (в JSON variables.tags может отсутствовать или быть пустым).
	Ожидается.Тест("Имя функционала").Что(ДанныеФайла).Получить("feature", "name").Равно("Функционал с тегами перед блоком Переменные");
