...
Процедура Тест_ФункционалGit(Ожидается) Экспорт
	...
	Ожидается.Тест("Статус после добавления").Что(ВК).Проц("Пауза", 100).Функц("status").Результат().Получить("index", 0, "new_name").Равно(ИмяФайла);
	...
	Ожидается.Тест("Статус после удаления").Что(ВК).Проц("Пауза", 100).Функц("status").Результат().Получить("work", 0, "new_name").Равно(ИмяФайла);
	Ожидается.Тест("Отмена изменений").Что(ВК).Функц("discard", ИмяФайла).Успешно();
	Ожидается.Тест("Статус после отмены").Что(ВК).Проц("Пауза", 100).Функц("status").Результат().Равно(Неопределено);
...
Процедура Тест_УвеличениеЭкрана(Ожидается) Экспорт
	...
	Ожидается.Тест("Левый угол увеличения").Что(Координаты).Получить("Left").Меньше(W / 3 + 1);
	Ожидается.Тест("Верхний угол увеличения").Что(Координаты).Получить("Top").Меньше(H / 3 + 1);
