...
Процедура Тест_ФункционалGit(Ожидается) Экспорт
	...
	Ожидается.Тест("Статус после добавления").Функц("status").Результат().Получить("index", 0, "new_name").Равно(ИмяФайла);
	...
	Ожидается.Тест("Статус после удаления").Функц("status").Результат().Получить("work", 0, "new_name").Равно(ИмяФайла);
	Ожидается.Тест("Отмена изменений").Что(ВК).Функц("discard", ИмяФайла).Успешно();
	Ожидается.Тест("Статус после отмены").Функц("status").Результат().Равно(Неопределено);
