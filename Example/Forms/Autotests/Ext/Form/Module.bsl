...
Процедура Тест_УвеличениеЭкрана(Ожидается) Экспорт
	...
	X = СвойстваДисплея.Width / 2;
	Y = СвойстваДисплея.Height / 2;
	
	Ожидается.Тест().Что(ВК).Проц("УстановитьПозициюКурсора",  X, Y);
	Ожидается.Тест().Что(ВК).Проц("УстановитьРазмерОкна", ИдентификаторОкна, X, Y);
	Ожидается.Тест().Что(ВК).Проц("УстановитьПозициюОкна", ИдентификаторОкна, X / 2, Y / 2);
	СнимокОкна = Ожидается.Тест().Что(ВК).Функц("ПолучитьСнимокЭкрана", 1).ЭтоКартинка().Вернуть();
	
	Ожидается.Тест().Что(ВК).Проц("ПоказатьУвеличение", X / 4, Y / 4, X, Y, 1);
	Ожидается.Тест().Что(ВК).Проц("Пауза", 100);
	Координаты = Ожидается.Тест().Что(ВК).Функц("НайтиНаЭкране", СнимокОкна).JSON().Вернуть();
	Ожидается.Тест("Фрагмент увеличения найден").Что(Координаты).Получить("Match").Больше(0.9);
	Ожидается.Тест("Левый угол увеличения").Что(Координаты).Получить("Left").Меньше(X / 3);
	Ожидается.Тест("Верхний угол увеличения").Что(Координаты).Получить("Top").Меньше(Y / 3);
	
	Ожидается.Тест().Что(ВК).Проц("СкрытьУвеличение");
	Ожидается.Тест().Что(ВК).Проц("Пауза", 100);
	Координаты = Ожидается.Тест().Что(ВК).Функц("НайтиНаЭкране", СнимокОкна).JSON().Вернуть();
	Ожидается.Тест("Фрагмент окна найден").Что(Координаты).Получить("Match").Больше(0.9);
	Ожидается.Тест("Левый угол окна").Что(Координаты).Получить("Left").Больше(X / 3);
	Ожидается.Тест("Верхний угол окна").Что(Координаты).Получить("Top").Больше(Y / 3);
