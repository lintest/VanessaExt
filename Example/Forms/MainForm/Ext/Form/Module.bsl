...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЦветТекста = ЦветаСтиля.ЦветТекстаПодсказки;
	ЦветФона = ЦветаСтиля.ЦветФонаПодсказки;
	ЦветКнопки = WebЦвета.ЛососьТемный;
	ЦветНадписи = WebЦвета.Белый;
	ЦветИконки = WebЦвета.Красный;
	ШрифтПодсказки = Новый Шрифт("Calibri", 12);
	Прозрачность = 127;
	Задержка = 20;
	Таймаут = 3000;
	Толщина = 4;
...
Процедура НайтиФайлыКомпонентой(Команда)
	...
	ВнешняяКомпонента.НачатьВызовНайтиФайлы(ОписаниеОповещения, Директория, МаскаПоиска, ИскомыйТекст, ИгнорироватьРегистр, ИскатьПапки);
...
Процедура МедиаВоспроизвести(Команда)
	...
	ВнешняяКомпонента.НачатьВызовВоспроизвестиМедиа(Новый ОписаниеОповещения, МультимедиаФайл, ИдентификаторМедиа);
...
Процедура МедиаПрервать(Команда)
	...
	ВнешняяКомпонента.НачатьВызовПрерватьМедиа(Новый ОписаниеОповещения, ИдентификаторМедиа);
...
Функция ПолучитьНастройкиРисования(ИспользуемКнопки = Ложь)
	...
	МассивКнопок.Добавить(Новый Структура("title", "Пропустить"));
	МассивКнопок.Добавить(Новый Структура("title", "Предыдущий шаг"));
	МассивКнопок.Добавить(Новый Структура("title", "Следующий шаг"));
	...
	ВремСтруктура.Вставить("fontName", ШрифтПодсказки.Имя);
	ВремСтруктура.Вставить("fontSize", ШрифтПодсказки.Размер);
...
Процедура ФигураЗатенение(Команда)
	...
	РазмерОкна = ВнешняяКомпонента.ПолучитьРазмерОкна(0);
	РазмерОкна = ПрочитатьСтрокуJSON(РазмерОкна);
	
	НастройкиРисования = ПолучитьНастройкиРисования();
	ВнешняяКомпонента.НачатьВызовНарисоватьТень(
		Новый ОписаниеОповещения, НастройкиРисования,
		РазмерОкна.Left, РазмерОкна.Top, РазмерОкна.Width, РазмерОкна.Height, ТекстНадписи);
	
КонецПроцедуры

&НаКлиенте
Процедура ФигураSpeechBubble(Команда)
	...
	ВремСтруктура.Вставить("duration", Таймаут);
	ВремСтруктура.Вставить("thickness", Толщина);
	ВремСтруктура.Вставить("tailWidth", ХвостШирина);
	ВремСтруктура.Вставить("tailLength", ХвостДлинна);
	ВремСтруктура.Вставить("tailRotation", ХвостУгол);
	ТекстJSON = ЗаписатьСтрокуJSON(ВремСтруктура);
	
	ВнешняяКомпонента.НачатьВызовНарисоватьТекст(
		Новый ОписаниеОповещения, ТекстJSON,
		ФигураX, ФигураY, ФигураW, ФигураH, ТекстНадписи);

КонецПроцедуры

&НаКлиенте
Процедура ФигураПоказатьТекст(Command)
	...
	ВнешняяКомпонента.НачатьВызовПоказатьТекст(
		Новый ОписаниеОповещения, ТекстJSON,
		ФигураX, ФигураY, ТекстНадписи);

КонецПроцедуры

&НаКлиенте
Процедура ФигураПоказатьПодсказку(Command)
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если Событие = "FOCUS_CHANGED" Тогда
		Текст = "Внешнее событие: " + Источник + " - " + Событие + "
		|" + Данные;
		ТекстКонсоли = Текст;
	Иначе
		Текст = "Внешнее событие: " + Источник + ",  " + Событие + ",  " + Данные;
		Сообщить(Текст);
	КонецЕсли;
...
Процедура НайтиЭлементы(Команда)
	...
	ВремСтруктура = Новый Структура("ProcessId", ИдентификаторПроцесса);
	...
	Если Не ЗначениеЗаполнено(ТипЭлемента) Тогда
		ВремСтруктура.Вставить("ControlType", ТипЭлемента);
	КонецЕсли;
	
	ТекстJSON = ЗаписатьСтрокуJSON(ВремСтруктура);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("НайденыЭлементыФормы", ЭтаФорма);
	ВнешняяКомпонента.НачатьВызовНайтиЭлементы(ОписаниеОповещения, ТекстJSON);
...
&НаСервереБезКонтекста
Функция ПолучитьКартинкуНаСервере(УникальныйИдентификатор, Масштаб)
...
Процедура НайтиКнопку(Команда)
	...
	ДополнительныеПараметры = Новый Структура("НомерКартинки,Left,Top,Width,Height,Handle", НомерКартинки);
	ЗаполнитьЗначенияСвойств(ДополнительныеПараметры, ТекСтр);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученСнимокОкнаЭлемента", ЭтаФорма, ДополнительныеПараметры);
	ВнешняяКомпонента.НачатьВызовПолучитьСнимокОкна(ОписаниеОповещения, ТекСтр.Handle);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолученСнимокОкнаЭлемента(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	
	ДополнительныеПараметры.Вставить("СнимокОкна", РезультатВызова);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученРазмерОкнаЭлемента", ЭтаФорма, ДополнительныеПараметры);
	ВнешняяКомпонента.НачатьВызовПолучитьРазмерОкна(ОписаниеОповещения, ДополнительныеПараметры.Handle);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолученРазмерОкнаЭлемента(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	
	РазмерыОкна = ПрочитатьСтрокуJSON(РезультатВызова);
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученФрагментЭлемента", ЭтаФорма, ДополнительныеПараметры);
	ВнешняяКомпонента.НачатьВызовОбрезатьИзображение(ОписаниеОповещения
		, ДополнительныеПараметры.СнимокОкна
		, ДополнительныеПараметры.Left - РазмерыОкна.Left
		, ДополнительныеПараметры.Top - РазмерыОкна.Top
		, ДополнительныеПараметры.Width
		, ДополнительныеПараметры.Height);
...
Процедура ПолученФрагментЭлемента(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученМасштабЭкрана", ЭтотОбъект, ДополнительныеПараметры);
	ВнешняяКомпонента.НачатьВызовПолучитьМасштаб(ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолученМасштабЭкрана(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	
	ОписаниеКартинки = ПолучитьКартинкуНаСервере(УникальныйИдентификатор, РезультатВызова);
...
&НаКлиенте
Процедура АктивныйЭлемент(Команда)

	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученАктивныйЭлемент", ЭтаФорма);
	ВнешняяКомпонента.НачатьПолучениеАктивныйЭлемент(ОписаниеОповещения);
...
&НаКлиенте
Процедура НажатьНаЭлемент(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения;
	ВнешняяКомпонента.НачатьВызовВызватьЭлемент(ОписаниеОповещения, ТекСтр.id);
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьЗначениеЭлемента(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения;
	ВнешняяКомпонента.НачатьВызовУстановитьЗначениеЭлемента(ОписаниеОповещения, ТекСтр.id, ЗначениеЭлемнта);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьЗначениеЭлемента(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученоЗначениеЭлемента", ЭтаФорма);
	ВнешняяКомпонента.НачатьВызовПолучитьЗначениеЭлемента(ОписаниеОповещения, ТекСтр.id);
	
КонецПроцедуры

&НаКлиенте
Процедура АктивироватьЭлемент(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения;
	ВнешняяКомпонента.НачатьВызовАктивироватьЭлемент(ОписаниеОповещения, ТекСтр.id);
	
КонецПроцедуры

&НаКлиенте
Процедура ПолученоЗначениеЭлемента(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	
	ЗначениеЭлемнта = РезультатВызова;
	
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьСледующийЭлемент(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученСоседнийЭлемент", ЭтаФорма);
	ВнешняяКомпонента.НачатьВызовПолучитьСледующийЭлемент(ОписаниеОповещения, ТекСтр.id);

КонецПроцедуры

&НаКлиенте
Процедура ПолучитьПредыдущийЭлемент(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученСоседнийЭлемент", ЭтаФорма);
	ВнешняяКомпонента.НачатьВызовПолучитьПредыдущийЭлемент(ОписаниеОповещения, ТекСтр.id);

КонецПроцедуры

&НаКлиенте
Процедура ПолученСоседнийЭлемент(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура UI_Поиск1(Команда)
	

	ВремСтруктура = Новый Структура("ProcessId", ИдентификаторПроцесса);
	ВремСтруктура.Вставить("LegacyIAccessibleHelp", "Имя элемента для поиска");
	ТекстJSON = ЗаписатьСтрокуJSON(ВремСтруктура);
	ОписаниеОповещения = Новый ОписаниеОповещения("НайденыЭлементыФормы", ЭтаФорма);
	ВнешняяКомпонента.НачатьВызовНайтиЭлементы(ОписаниеОповещения, ТекстJSON);
...
&НаКлиенте
Процедура ПоказатьОкноПрерывания(Команда)
	
	Текст = СтрРазделить(ТекстНадписи, Символы.ПС)[0];
	...
	ВнешняяКомпонента.НачатьВызовПоказатьОкноПрерывания(
		Новый ОписаниеОповещения, ТекстJSON, Текст, "Стоп");

КонецПроцедуры

&НаКлиенте
Процедура ЗакрытьОкноПрерывания(Команда)

	ВнешняяКомпонента.НачатьВызовЗакрытьОкноПрерывания(Новый ОписаниеОповещения);
...
&НаКлиенте
Процедура ЗатенениеКнопки(Команда)

	НастройкиРисования = ПолучитьНастройкиРисования(Истина);
	ВнешняяКомпонента.НачатьВызовНарисоватьТень(
		Новый ОписаниеОповещения, НастройкиРисования,
		ФигураX, ФигураY, ФигураW, ФигураH, ТекстНадписи);
...
Процедура МониторингСобытийНачать(Команда)
	...
	ВнешняяКомпонента.НачатьУстановкуМониторингАктивногоЭлемента(Новый ОписаниеОповещения, Истина);

КонецПроцедуры

&НаКлиенте
Процедура МониторингСобытийЗавершить(Команда)

	ВнешняяКомпонента.НачатьУстановкуМониторингАктивногоЭлемента(Новый ОписаниеОповещения, Ложь);
