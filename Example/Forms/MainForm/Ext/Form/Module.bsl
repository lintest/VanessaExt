...
Функция ПолучитьКартинкуНаСервере(УникальныйИдентификатор, ВысотаЭлемента)
	...
	Для каждого ОписаниеКартинки из ВремТабл Цикл
		...
		Если ПредыдущаяВысотаКартинки < ВысотаЭлемента И ВысотаЭлемента <= ВысотаКартинки Тогда
			...
			Прервать;
		Иначе
			ПредыдущаяВысотаКартинки = ВысотаКартинки;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПолученФрагментЭлемента(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ОписаниеКартинки = ПолучитьКартинкуНаСервере(УникальныйИдентификатор, ДополнительныеПараметры.Height);
...
Процедура НайденФрагментЭлемента(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	...
	ВнешняяКомпонента.НачатьВызовНарисоватьПрямоугольник(
		Новый ОписаниеОповещения, НастройкиРисования, 
		ДополнительныеПараметры.Left + Координаты.Left, 
		ДополнительныеПараметры.Top + Координаты.Top, 
		Координаты.Width, Координаты.Height);
