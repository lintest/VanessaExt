...
Процедура ПолучитьСписокФайлов(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолученТекстБуфераОбмена", ЭтотОбъект);
	БуферОбмена.НачатьПолучениеФайлы(ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьТекстКартинку(Команда)

	ОписаниеОповещения = Новый ОписаниеОповещения("ПоместитьКартинкуТекст", ЭтотОбъект);
	БуферОбмена.НачатьВызовОчистить(ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПоместитьКартинкуТекст(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	
	ДвоичныеДанные = ПолучитьИзВременногоХранилища(КартинкаЛоготип1С);
	БуферОбмена.НачатьВызовЗаписатьКартинку(Новый ОписаниеОповещения, ДвоичныеДанные);
	БуферОбмена.НачатьВызовЗаписатьТекст(Новый ОписаниеОповещения, "Логотип 1С");
	
КонецПроцедуры	

&НаКлиенте
Процедура ПоместитьФайлыВБуфер(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыбраныФайлы", ЭтотОбъект);
...
Процедура ВыбраныФайлы(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ВыбранныеФайлы) = Тип("Массив") Тогда
		...
		ЗаписатьJSON(ЗаписьJSON, ВыбранныеФайлы);
		СтрокаJSON = ЗаписьJSON.Закрыть();
		БуферОбмена.НачатьУстановкуФайлы(Новый ОписаниеОповещения, СтрокаJSON);
	КонецЕсли;		
