...
Процедура ЗапуститьБраузер(Команда)
	...
	ФайлПриложения = Новый Файл(ProgramFiles + ИмяФайла);
	Если Не ФайлПриложения.Существует() Тогда
		...
		КонецЕсли;
	КонецЕсли;
	СтрокаКоманды = """" + ФайлПриложения.ПолноеИмя + """ --remote-debugging-port=9222 about:blank";
	НачатьЗапускПриложения(Новый ОписаниеОповещения, СтрокаКоманды);
	
КонецПроцедуры

&НаКлиенте
Процедура СтартВебКлиента(Команда)
...
Процедура ВыполнитьJavaSctipt(Команда)
	...
	ВнешняяКомпонента.НачатьВызовВебСокет(ОписаниеОповещения, АдресВебСокет, ТекстJSON);
