&НаКлиенте
Перем ИдентификаторКомпоненты, ВнешняяКомпонента, БуферОбмена, ЭкземплярПроцесса;
...
Процедура СтартоватьПроцесс(Команда)
	...
	ЭкземплярПроцесса = Новый("AddIn." + ИдентификаторКомпоненты + ".ProcessControl");
	ЭкземплярПроцесса.Создать(КоманднаяСтрока, Истина);
